...
then 
    ...
    cd packages
    mv `ls | grep "veles-desktop.*linux-x64.zip"` veles-desktop-linux-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    mv `ls | grep "veles-desktop.*linux-amd64.deb"` veles-desktop-linux-amd64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.deb
    cd ..
...
then 
    ...
    cd packages
    mv `ls | grep "veles-desktop.*mac.zip"` veles-desktop-mac-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    cd ..
...
#    echo disable > "${WINEPREFIX:-${HOME}/.wine}/.update-timestamp"
    ...
    cd packages
    zip -r veles-desktop-win-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-unpacked
    cd ..

    DEBUG=electron-builder yarn run travis:win32

    cd packages
    zip -r veles-desktop-win-ia32-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-ia32-unpacked
    cd ..
...
then 
    ...
    for fn in `ls | grep "veles-desktop"`; do
        echo "Uploading $fn"
